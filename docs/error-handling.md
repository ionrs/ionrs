...
This workspace disables `redacted-error`'s optional static-string obfuscation
backend so the dependency tree does not include a direct or transitive string
obfuscation crate. Callers must use the `redacted_error` API rather than a
backend-specific API.

Do not add direct string-obfuscation dependencies. Name hiding is handled by
compile-time hashing and debug-only name registration.

## String Sensitivity

Release-safe public wording is for strings that reveal implementation,
protocol, or operator intent. It is not a blanket rule for every literal.

Static string hiding only reduces static binary inspection. It does not make a
string safe to show in `Display`, `PublicError`, logs, or serialized API
responses.

Do not wrap empty strings, 1-character strings, or one-word strings that are not
sensitive. Use plain literals or `redacted_error::Message::from_static` for
those cases when a `Message` value is required.

Never expose protocol strings, function names, method names, and Ion script
binding names in release-facing errors. This includes wire keys, service names,
RPC method names, function or method identifiers, host binding names, and
strings that describe offensive security behavior. Use compile-time hashes,
debug-only registration, structured private codes, or generic public wording
instead.
...
- Use `redacted_error::message_string!` only when an owned public string is
  required.
- Do not wrap empty strings, 1-character strings, or one-word non-sensitive
  strings.
- Do not expose protocol strings, function names, method names, Ion binding
  names, or offensive security terms in release-facing messages.
- Do not add direct string-obfuscation dependencies.
